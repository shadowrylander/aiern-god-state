{
  ...
  description = "An aiern-mode state for using god-mode.";
  inputs = rec {
    bundle = {
    ...
    ePkg-aiern.url =
      "git+https://github.com/syvlorg/aiern.git";
  ...
  outputs = inputs@{ self, flake-utils, ... }:
    ...
    inputs.bundle.mkOutputs.emacs {
      inherit inputs self;
      pname = "aiern-god-state";
      callPackage = { emacs }:
        iron.mkEmacsPackage {
          inherit self inputs emacs;
          package = rec {
            src = ./.;
            propagatedUserEnvPkgs = toList emacs.pkgs.god-mode;
            meta.description = "An aiern-mode state for using god-mode.";
          };
        };
    } { };
}
